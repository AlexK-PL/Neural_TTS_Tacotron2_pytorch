...
# from distribu import apply_gradient_allreduce
...
def train(output_directory, log_directory, checkpoint_path, warm_start, n_gpus, rank, group_name,
    ...
    model.train()
    print("Batch size is: {} samples".format(hyper_params['batch_size']))
    print("Learning rate is: {} samples".format(hyper_params['learning_rate']))
    ...
    for epoch in range(epoch_offset, hyper_params['epochs']):
        ...
        for i, batch in enumerate(train_loader):
            ...
            _, input_lengths, _, _, _ = input_data
            output_predicted = model(input_data)

            loss = criterion(output_predicted, output_target, input_lengths, iteration)  # MODIFIED!!
            ...
            if not overflow and (iteration % hyper_params['iters_per_checkpoint'] == 0):  # and iteration != 0
